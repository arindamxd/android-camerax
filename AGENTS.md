...
- Overlay Compose chrome on `PreviewView` with `ImplementationMode.COMPATIBLE` (TextureView). `PERFORMANCE` / SurfaceView steals touches so settings and footer buttons miss. Chrome uses `zIndex`, `WindowInsets.safeDrawing.union(systemGestures)`, and 48dp targets. Mode pager must tap-to-select, not only snap the centered item. Live-feed header icons and status chips are **mode-based** (Photo: flash/timer/grid/motion/exposure + Ultra HDR/RAW; Video: flash/grid/exposure + Stabilized/video HDR/60 fps/LLB, never Ultra HDR; Slo-mo: flash + fps chip; Effects: flash/timer/grid/exposure + filter strip; Pano: grid + pan hint, no zoom; Dual: flash/grid + Front+back chip, PiP at bottom-end, hide zoom/flip/timer/motion/exposure). Full Settings stay behind the gear.
- Live filters are **two pipelines**, not one API. CameraX effects: `ColorFilterProcessor` (`CameraEffect` / `SurfaceProcessor`) for Original / Mono / Invert / Sepia / Cool / Warm / Vivid; same color matrix on still JPEGs. Media3 effects: `Media3Effect` (`androidx.camera.media3`) for Bright / Contrast via `setEffects` (Media3 GPU `Brightness` / `Contrast`). Do not bind both on the same session. Rebind when enabling/disabling an effect or when switching matrix ↔ Media3; do not rebind between chips in the same pipeline. Skip recompress when the still is UltraHDR so gain maps survive.
